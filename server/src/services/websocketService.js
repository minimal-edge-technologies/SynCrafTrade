...
async handleOrderUpdate(orderData) {
  try {
    ...
    switch(status) {
      ...
      case 'FILLED':
      case 'EXECUTED':
        // We still want to track completed orders
        await this.copyOrderToChildren(orderData, childAccounts);
        break;
        
      default:
        console.log(`[ORDER] Unhandled order status: ${status}, orderId: ${orderData.orderId}`);
        // For unrecognized statuses, copy by default
        await this.copyOrderToChildren(orderData, childAccounts);
    }
  } catch (error) {
  ...
  async updateChildOrders(parentOrder, childAccounts) {
    // Find all related child orders and update them
    const childOrders = await Order.find({ parentOrderId: parentOrder.orderId });
    
    for (const childOrder of childOrders) {
      try {
        const childAccount = childAccounts.find(acc => acc._id === childOrder.accountId);
        ...
        // Modify child order
        await this.angelOneService.modifyOrder({
        ...
        console.error(`Failed to update child order ${childOrder.orderId}:`, error);
      }
    }
  }

  async cancelChildOrders(parentOrder, childAccounts) {
    // Find and cancel all related child orders
    const childOrders = await Order.find({ parentOrderId: parentOrder.orderId });
    
    for (const childOrder of childOrders) {
      try {
        await this.angelOneService.cancelOrder(childOrder.orderId);
      } catch (error) {
        console.error(`Failed to cancel child order ${childOrder.orderId}:`, error);
      }
    }
  }
